...
void parseExtDef(Node *ext_def) // specifier extEecList Semi || Specifier SEMI || specifier funDec compst
{
...
ParamsList *parseDefList(Node *def_list, ParamsList *paramslist) // 递归是否要使用参数i 然后调用i+1？ def edfList || null
{
    printDebugMsg("deflist");
    printDebugMsg("test");
    if (def_list->children.size() == 0)
    { // 无参
        printDebugMsg("DefList NULL");
        return paramslist;
    }
    Node *def = def_list->children[0];
    if (paramslist == nullptr)
    {
        printDebugMsg("param = null");
        paramslist = parseDef(def, paramslist);
    }
    else
    {
        printDebugMsg("param exit");
...
void parseStmt(Node *stmt, Attribute *attribute)
{
    ...
    else if (node1->type == NodeType::Return) // return exp semi
    {
        ...
        {
            std::cout << attribute->nodetype << std::endl;
